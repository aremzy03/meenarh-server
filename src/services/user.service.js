...
async function getCustomerById(id) {
  const [customers] = await pool.execute(
    'SELECT id, name, email, phone, default_address, is_email_verified, created_at, updated_at FROM customers WHERE id = ?',
    [id]
  );
  return customers.length > 0 ? customers[0] : null;
}
...
async function resetPasswordWithOtpByEmail({ email, code, newPassword }) {
